    ...
    <main style={{ background: 'var(--bg)', minHeight: '100vh', paddingTop: 88 }}>
          ...
          <div className="terminal" style={{ borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '0.75rem 1.5rem', background: 'var(--surface)', borderBottom: '1px solid rgba(6,182,212,0.08)' }}>
              {['User', 'Role', 'Department', 'Status', 'Last Active'].map(h => (
                <div key={h} className="mono" style={{ color: 'var(--t4)', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.08em' }}>{h}</div>
              ))}
            </div>
            {USERS.map((u, i) => (
              <div key={i} style={{
              ...
              >
                <div style={{ color: 'var(--t1)', fontWeight: 600, fontSize: '0.88rem' }}>{u.n}</div>
                <div>
                ...
                <div style={{ color: 'var(--t5)', fontSize: '0.83rem' }}>{u.d}</div>
                ...
                <div className="mono" style={{ color: 'var(--t4)', fontSize: '0.75rem' }}>{u.l}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ marginTop: '1.5rem', overflowX: 'auto' }}>
            <div className="terminal" style={{ borderRadius: 14, display: 'inline-block', minWidth: '100%' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '120px repeat(6, 1fr)', padding: '0.75rem 1.5rem', background: 'var(--surface)', borderBottom: '1px solid rgba(6,182,212,0.08)', gap: '0.5rem' }}>
                <div className="mono" style={{ color: 'var(--t4)', fontSize: '0.68rem', fontWeight: 700 }}>ROLE</div>
                {PERMS.map(p => <div key={p} className="mono" style={{ color: 'var(--t4)', fontSize: '0.65rem', fontWeight: 700, textAlign: 'center' }}>{p}</div>)}
              </div>
              {ROLES.map((row, i) => (
                <div key={i} style={{
                  ...
                  {row.p.map((v, pi) => (
                    <div key={pi} style={{ textAlign: 'center' }}>
                      {v ? <span style={{ color: '#10B981', fontSize: '1.1rem' }}>✓</span> : <span style={{ color: 'var(--t4)' }}>—</span>}
                    </div>
                  ))}
                </div>
              ))}
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem', alignItems: 'start' }}>
            <div>
              ...
              <p style={{ color: 'var(--t5)', lineHeight: 1.7, marginBottom: '1.5rem' }}>
              ...
              <div className="glass" style={{ padding: '1.25rem' }}>
                <div className="mono" style={{ color: 'var(--t4)', fontSize: '0.7rem', marginBottom: '0.25rem', letterSpacing: '0.08em' }}>TOTAL REDACTED — LAST 30 DAYS</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {PII_BARS.map((bar, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div style={{ flex: 1, background: 'var(--glass-bg)', borderRadius: 4, height: 6, overflow: 'hidden' }}>
                    <motion.div
                      initial={{ width: 0 }}
                  ...
                  <span className="mono" style={{ width: 40, textAlign: 'right', color: 'var(--t4)', fontSize: '0.72rem' }}>{bar.n.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding: '5rem 1.5rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--t5)', marginBottom: '2rem' }}>
