...
export default function Security() {
  return (
    <main style={{ background: '#02060E', minHeight: '100vh', paddingTop: 88 }}>
      ...
      <section className="section">
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.7rem, 3vw, 2.3rem)', fontWeight: 900, color: '#E2E8F0', margin: '1rem 0', letterSpacing: '-0.02em' }}>
              12+ PII Categories Detected & Redacted<br /><span style={{ color: '#F87171' }}>Before Indexing</span>
            </h2>
            <p style={{ color: '#475569', maxWidth: 580, margin: '0 auto', lineHeight: 1.7 }}>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {['Immutable log with cryptographic signing', 'User identity linked to every event', 'PII access attempts flagged & escalated', 'Exportable reports for regulatory submissions'].map((pt, i) => (
                  <li key={i} style={{ display: 'flex', gap: '0.5rem', color: '#64748B', fontSize: '0.875rem', alignItems: 'flex-start' }}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="terminal">
                <div className="terminal-bar">
                  ...
                  <span className="mono" style={{ color: '#334155', fontSize: '0.72rem', marginLeft: 8 }}>audit-log — live stream</span>
      ...
      <section className="section">
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.7rem, 3vw, 2.2rem)', fontWeight: 900, color: '#E2E8F0', marginTop: '1rem', letterSpacing: '-0.02em' }}>
          ...
          >
            {AUTH.map((a, i) => (
              <motion.div key={i} variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }} className="glass" style={{ padding: '1.75rem' }}>
                ...
                <h3 style={{ color: '#E2E8F0', fontWeight: 700, fontSize: '0.95rem', marginBottom: '0.5rem' }}>{a.t}</h3>
                <p style={{ color: '#475569', fontSize: '0.85rem', lineHeight: 1.6 }}>{a.d}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* Compliance badges */}
      <section style={{ padding: '4rem 1.5rem 6rem', background: '#020913', textAlign: 'center' }}>
        <hr className="divider" style={{ marginBottom: '4rem' }} />
        <div className="container">
          <span className="label">Regulatory Alignment</span>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#E2E8F0', margin: '1rem 0 2.5rem', letterSpacing: '-0.02em' }}>
